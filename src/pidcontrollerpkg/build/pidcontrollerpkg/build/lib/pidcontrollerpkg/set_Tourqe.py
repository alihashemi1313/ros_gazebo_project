...
class MinimalClientAsync(Node):
    ...
    def build_trajectory(self):
        ...
        trajectory1.q0 = self._theta[0]
        trajectory1.q1 = self.joint_in[0]
        trajectory1.v0 = 0
        trajectory1.v1 = 0
        trajectory1.v_max = 1.5
        trajectory1.a_max = 3
        trajectory1.j_max = 10
        trajectory1.dt = 0.001
        trajectory1.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory1.q):
                q1.append(trajectory1.q[i])
            else:
                q1.append(self.joint_in[0])
        ...
        trajectory2.q0 = self._theta[1]
        trajectory2.q1 = self.joint_in[1]
        trajectory2.v0 = 0
        trajectory2.v1 = 0
        trajectory2.v_max = 1.5 
        trajectory2.a_max = 3
        trajectory2.j_max = 10
        trajectory2.dt = 0.001
        trajectory2.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory2.q):
                q2.append(trajectory2.q[i])
            else:
                q2.append(self.joint_in[1])
        ...
        trajectory3.q0 = self._theta[2]
        trajectory3.q1 = self.joint_in[2]
        trajectory3.v0 = 0
        trajectory3.v1 = 0
        trajectory3.v_max = 1.5
        trajectory3.a_max = 3
        trajectory3.j_max = 10
        trajectory3.dt = 0.001
        trajectory3.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory3.q):
                q3.append(trajectory3.q[i])
            else:
                q3.append(self.joint_in[2])
        ...
        trajectory4.q0 = self._theta[3]
        trajectory4.q1 = self.joint_in[3]
        trajectory4.v0 = 0
        trajectory4.v1 = 0
        trajectory4.v_max = 1.5
        trajectory4.a_max = 3
        trajectory4.j_max = 10
        trajectory4.dt = 0.001
        trajectory4.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory4.q):
                q4.append(trajectory4.q[i])
            else:
                q4.append(self.joint_in[3])
        ...
        trajectory5.q0 = self._theta[4]
        trajectory5.q1 = self.joint_in[4]
        trajectory5.v0 = 0
        trajectory5.v1 = 0
        trajectory5.v_max = 1.5
        trajectory5.a_max = 3
        trajectory5.j_max = 10
        trajectory5.dt = 0.001
        trajectory5.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory5.q):
                q5.append(trajectory5.q[i])
            else:
                q5.append(self.joint_in[4])
        ...
        trajectory6.q0 = self._theta[5]
        trajectory6.q1 = self.joint_in[5]
        trajectory6.v0 = 0
        trajectory6.v1 = 0
        trajectory6.v_max = 1.5 
        trajectory6.a_max = 3
        trajectory6.j_max = 10
        trajectory6.dt = 0.001
        trajectory6.BuildTrajectory()
        for i in range(0,int(self.a_time*1000+1)):
            if i<len(trajectory6.q):
                q6.append(trajectory6.q[i])
            else:
                q6.append(self.joint_in[5])
    ...
    def send_request(self):
        ...
        self.joint_in = [0]*6
        print("Enter joint position matrix(6*1):")
        self.joint_in = [float(input()) for x in range (6)]
        ...
        while self.counter <= Decimal(5.000) :   # total time 
            self.future_step = self.step_call_cli.call_async(self.req)
            rclpy.spin_until_future_complete(self, self.future_step)
            self.res = self.future_step.result()
            ...
            self._theta[5] = self.res.r6
            for i in range(6):
                self.error[i] = self.ftraj[i][n] - self._theta[i]
